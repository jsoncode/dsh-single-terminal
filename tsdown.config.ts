/**
 * dsh-single-terminal —— tsdown 构建配置（对齐 dsh-jenkins 的 client bundle 方案）。
 *
 * 两个产物：
 * - lib/index.js  宿主半边（Node，ESM）：src/host/index.ts 打包；
 *   dependencies（node-pty / ws）与 @deepseek-ai/* 保持 external（node-pty 为原生
 *   模块不可打包，运行时从插件目录 node_modules 解析）；devDeps 全部内联。
...
const nodeConfig: UserConfig = {
  ...
  // node-pty（原生模块）与 ws 保持 external，运行时经插件目录 node_modules 解析。
  deps: {
    neverBundle: ['node-pty', 'ws'],
  },
}

const clientConfig: UserConfig = {
  name: 'dsh-single-terminal/client',
  ...
  deps: {
    ...
    // 其余依赖（@xterm/* 等）全部内联，避免 factory require 表外模块抛错。
  ...
  banner: `window.__ModuleLoader__.load({ id: 'dsh-single-terminal', factory: (require) => {
